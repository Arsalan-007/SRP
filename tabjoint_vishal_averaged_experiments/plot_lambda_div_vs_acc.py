...
SAVE_PNG = "/media/homes/hamza/results/Lambda_div_exp/lambda_div_vs_test_accuracy_titanic.png"
# ...existing code...
...
def extract_lambda_cka_and_test_accuracy(results: List[dict]) -> Tuple[List[float], List[float]]:
    ...
    for r in results:
        ...
        lambda_div = None
        if "lambda_div" in r:
            lambda_cka = r["lambda_div"]
        else:
            for container in ("hparams", "config", "params"):
                if container in r and isinstance(r[container], dict) and "lambda_div" in r[container]:
                    lambda_div = r[container]["lambda_div"]
                    break
        ...
        if lambda_div is None or test_acc is None:
            # skip runs missing either field
            continue

        try:
            pairs.append((float(lambda_div), float(test_acc)))
        except Exception:
            continue
...
def plot_lambda_cka_vs_accuracy(lambda_cka_vals, accuracies, save_path: str = None, show: bool = True):
    ...
    plt.xlabel("lambda_div")
    plt.ylabel("Test Accuracy")
    plt.title("lambda_div vs Test Accuracy")
    plt.grid(True)
    plt.tight_layout()
    if save_path:
